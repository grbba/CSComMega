...
auto DccExInterface::recieve() -> void
{
    if (!DCCI.getQueue(IN)->isEmpty()) {
        ...
        if( m.sta == sta ) {
            ERR(F("Wrong sender; Msg seems to have been send to self; Msh has been ignored" CR));
            return;
        }
        ...
        case _WITHROTTLE:
        {
            ERR(F("WiThrottle not yet supported. Message ignored."));
            break;
        } // Withrottle valid only if the sta which send is NW i.e. the CS is the reciever
        case _CTRL:
        {
            ERR(F("Ctrl messages not yet supported. Message ignored"));
            break;
        ...
        case _REPLY:
        {
            ERR(F("Processing reply from the CommandStation..."));
            break;
...
void DccExInterface::queue(uint16_t c, uint8_t p, char *msg)
{
    ...
    INFO("Queuing [%s:%d:%d:%s]:[%s]" CR, decode(sta), m.mid, m.client, decode((csProtocol)m.p), m.msg.c_str());
